...
<h4 class="my">
    <small>
        <div><i class="fa fa-calendar" aria-hidden="true"></i> <{$smarty.const._MB_TAD_SIGNUP_ACTION_DATE}><{$smarty.const._TAD_FOR}><{$block.action_date|substr:0:-3}></div>
...
<div class="text-center my-3">
    <a href="<{$xoops_url}>/modules/beck_signup/index.php?op=beck_signup_data_create&action_id=<{$block.id}>" class="btn btn-lg btn-info 
        <{if !($block.enable &&  ($block.number + $block.candidate)  > $block.signup_count && $xoops_isuser && $block.end_date|strtotime >= $smarty.now)}>disabled<{/if}>"><i class="fa fa-plus" aria-hidden="true"></i> <{$smarty.const._MB_TAD_SIGNUP_APPLY_NOW}></a>
</div>